...
# ============================================================
# Submit NeighborCache paper experiments to Slurm as an array
# Conda-env version: uses the `ec` environment by default.
# ============================================================
#
# Usage:
#   chmod +x submit_neighborcache_paper_experiments_ec.sh
#   ./submit_neighborcache_paper_experiments_ec.sh
#
# Dry run:
#   DRY_RUN=1 ./submit_neighborcache_paper_experiments_ec.sh
#
# Override env name:
#   CONDA_ENV_NAME=myenv ./submit_neighborcache_paper_experiments_ec.sh
#
# This script resolves the absolute Python executable from the Conda env
# at submit time and writes that executable into every experiment command.
# That is more robust than depending on Slurm inheriting `conda activate`.
#
# ============================================================
...
resolve_conda_python() {
  ...
  # Preferred: use conda run to ask the env which Python it uses.
  if command -v conda >/dev/null 2>&1; then
    conda run -n "${env_name}" python -c 'import sys; print(sys.executable)' 2>/dev/null && return 0
  fi

  # Fallback: source common conda init files, then try again.
  for conda_sh in \
...
if [[ -z "${PYTHON_BIN:-}" ]]; then
  ...
  if ! PYTHON_BIN="$(resolve_conda_python "${CONDA_ENV_NAME}")"; then
    ...
    echo "  conda run -n ${CONDA_ENV_NAME} python -c 'import sys; print(sys.executable)'"
    echo "  PYTHON_BIN=/absolute/path/to/ec/bin/python ./submit_neighborcache_paper_experiments_ec.sh"
    exit 2
  fi
fi
...
# Validate required packages before submitting.
"${PYTHON_BIN}" - <<'PY'
import sys
...
if missing:
    print("ERROR: Missing required Python packages in selected environment:", file=sys.stderr)
...
# Usually unnecessary because commands use absolute env Python.
# You can still pass ENV_SETUP if your cluster needs modules loaded.
ENV_SETUP="${ENV_SETUP:-}"
...
if [[ "${N_TASKS}" -eq 0 ]]; then
  echo "No tasks were generated. Check RUN_* toggles."
  exit 1
fi

{
  ...
  echo "full_n_train=${FULL_N_TRAIN}"
  echo "full_n_calib=${FULL_N_CALIB}"
  echo "full_n_eval=${FULL_N_EVAL}"
  echo "smoke_trials=${SMOKE_TRIALS}"
  echo "main_trials=${MAIN_TRIALS}"
  echo "ablation_trials=${ABLATION_TRIALS}"
  echo "alphas=${ALPHAS[*]}"
  echo "seeds=${SEEDS[*]}"
  echo "train_sizes=${TRAIN_SIZES[*]}"
  echo "calib_sizes=${CALIB_SIZES[*]}"
  echo "tau_modes=${TAU_MODES[*]}"
